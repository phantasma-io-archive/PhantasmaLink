...
namespace Phantasma.SDK
{
	...
	public struct Nexus 
	{
		...
		public static Nexus FromNode(DataNode node) 
		{
			...
			var masters_array = node.GetNode("masters");
			if (masters_array != null) {
				result.masters = new string[masters_array.ChildCount];
				for (int i=0; i < masters_array.ChildCount; i++) {
											
					result.masters[i] = masters_array.GetNodeByIndex(i).AsString();
				}
			}
			else {
				result.masters = new string[0];
			}
						
			var ses_array = node.GetNode("ses");
			if (ses_array != null) {
				result.ses = new Leaderboard[ses_array.ChildCount];
				for (int i=0; i < ses_array.ChildCount; i++) {
					
					result.ses[i] = Leaderboard.FromNode(ses_array.GetNodeByIndex(i));
					
				}
			}
			else {
				result.ses = new Leaderboard[0];
			}
	...
	public struct Account 
	{
		...
		public static Account FromNode(DataNode node) 
		{
			...
			result.name = node.GetString("name");						
			result.stake = Stake.FromNode(node.GetNode("stake"));						
		...
		public static Script FromNode(DataNode node) 
		{
			...
			if (results_array != null) {
				...
				for (int i=0; i < results_array.ChildCount; i++) {
					...
					result.results[i] = results_array.GetNodeByIndex(i).AsString();
				}
			}
			else {
   ...
   public class API {	   
		...
		public void GetNexus(Action<Nexus> callback, Action<EPHANTASMA_SDK_ERROR_TYPE, string> errorHandlingCallback = null)  
		{	   
